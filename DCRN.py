...
class DCRN(nn.Module):
    ...
    def forward(self, X_tilde1, Am, X_tilde2, Ad):
        ...
        Z_l = torch.spmm(Am, Z_i)  ##表示对 ZI 局部结构增强，Am正则化的邻接矩阵
        S = torch.mm(Z_l, Z_l.t())
        S = F.softmax(S, dim=1)
        Z_g = torch.mm(S, Z_l)  ##计算全局相关性
        Z = self.alpha * Z_g + Z_l  ##聚类嵌入

        # AE decoding
        X_hat = self.ae.decoder(Z)  ##Rebuilt attribute matrix

        # IGAE decoding
        Z_hat, Z_adj_hat, AZ_de, Z_de = self.gae.decoder(Z, Am)  ##z_hat, z_hat_adj, [az_1, az_2, az_3], [z_1, z_2, z_hat]
        sim = (A_igae1 + A_igae2) / 2  ##gae.encoder重建的A矩阵
        A_hat = sim + Z_adj_hat   ##Rebuilt adjacency matrix
